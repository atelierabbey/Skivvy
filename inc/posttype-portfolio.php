...
//[portfolio cat="pond-water-features" show="20" orderby="date" orderin="asc"]
function portfoliosc($atts){
	...
	foreach( $myposts as $post ) :
		setup_postdata($post);
		$image_id = get_post_thumbnail_id($post->ID,"thumbnail");
		$image_url = wp_get_attachment_image_src($image_id,'large', true);
		if ( has_post_thumbnail() ) { 
			return '<div class="pstthmb"><a href="'.$image_url[0].'" rel="shadowbox[port-'.$post->portfolio_category.']">'.get_the_post_thumbnail($post->ID,"thumbnail").'</a></div>';
		} 
	endforeach; //*/
}
add_shortcode( 'portfolio', 'portfoliosc' );
