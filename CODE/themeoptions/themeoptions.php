...
function fburl_sc( $atts ){$options = get_option( 'sample_theme_options' );
	return '<a class="btn_facebook" href="'.$options["fburl"].'" target="_blank"><img src="'.get_bloginfo("template_url").'/img/clear.gif" alt="Facebook - '.$options["fburl"].'"></a>';
} add_shortcode( 'btn_fb', 'fburl_sc' );

// Twitter Shortcode
function twurl_sc( $atts ){$options = get_option( 'sample_theme_options' );
	return '<a class="btn_twitter" href="'.$options["twurl"].'" target="_blank"><img src="'.get_bloginfo("template_url").'/img/clear.gif" alt="Twitter - '. $options["twurl"].'"></a>';
} add_shortcode( 'btn_tw', 'twurl_sc' );

// Linkedin Shortcode
function liurl_sc( $atts ){$options = get_option( 'sample_theme_options' );
	return '<a class="btn_linkedin" href="'.$options["liurl"].'" target="_blank"><img src="'.get_bloginfo("template_url").'/img/clear.gif" alt="Linkedin - '.$options["liurl"].'"></a>';
} add_shortcode( 'btn_li', 'liurl_sc' );

// Youtube Shortcode
function yturl_sc( $atts ){$options = get_option( 'sample_theme_options' );
	return '<a class="btn_youtube" href="'.$options["yturl"].'" target="_blank"><img src="'.get_bloginfo("template_url").'/img/clear.gif" alt="Youtube -'.$options["yturl"].'"></a>';
} add_shortcode( 'btn_yt', 'yturl_sc' );

// Phone Shortcode
function phtxt_sc( $atts ){$options = get_option( 'sample_theme_options' );
	return '<span class="txt_phone">'.$options["phtxt"].'</span>';
} add_shortcode( 'txt_ph', 'phtxt_sc' );

// Phone Shortcode
function adtxt_sc( $atts ){$options = get_option( 'sample_theme_options' );
	return '<span class="txt_address">'.$options["adtxt"].'</span>';
